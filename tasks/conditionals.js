...
function doRectanglesOverlap(rect1, rect2) {
  // Check if rect1 is to the left of rect2 or vice versa
  if (
  ...
  // Check if rect1 is above rect2 or vice versa
  if (
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  // Determine the opening and closing brackets based on inclusion flags
  ...
  const intervalString = `${startBracket}${start}, ${end}${endBracket}`;

  return intervalString;
}
console.log(getIntervalString(0, 1, false, false));
...
function isCreditCardNumber(ccn) {
  // Convert the credit card number to a string for easy manipulation
  ...
  // A valid CCN should have a sum that is a multiple of 10
  return sum % 10 === 0;
}
...
function toNaryString(num, n) {
  if (n < 2 || n > 10) {
    throw new Error("Base number must be between 2 and 10");
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  // Split each path into an array of directory components
  ...
  // Iterate through each directory component at the same index and compare
  let commonPath = "";
  for (let i = 0; i < minLength; i++) {
    const componentToCheck = pathComponents[0][i];

    if (
      pathComponents.every((components) => components[i] === componentToCheck)
    ) {
      commonPath += componentToCheck + "/";
    } else {
